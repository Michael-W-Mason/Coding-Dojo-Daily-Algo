...
for(var i=0; i<pokémon.length; i++) {
    //1st challenge
    if(pokémon[i].id % 3 == 0){
        consoleOut["1"].push(pokémon[i]);
    }
    //2nd Challenge
    if(pokémon[i].types.length > 1){
        consoleOut["2"].push(pokémon[i]);
    }
    //3rd Challenge
    if(pokémon[i].types == "poison"){
        consoleOut["3"].push(pokémon[i].name);
    }
    //4th Challenge
    if(pokémon[i].types[1] == "flying"){
        consoleOut["4"].push(pokémon[i].types[0]);
    }
    //Bonus
    if(pokémon[i].types == "poison"){
        consoleOut["bonus"].push(pokémon[i].name.split("").reverse().join(""));
    }
}
